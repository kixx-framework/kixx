...
// Every partial the application ships is flat, but both store adapters document
// the `partials/` tree as recursive, and loadPartials() derives the include name
// from whatever remains after the prefix — so a nested partial is reachable as
// `{{>e2e/nested/partial-template.html}}`. Publishing one exercises that
// documented shape and the multi-segment wildcard join at the same time.
const TEMPLATE_FILEPATH = 'e2e/nested/partial-template.html';

// A single-segment filepath, the shape every application partial actually uses.
...
const FLAT_TEMPLATE_FILEPATH = 'e2e-overwrite-target.html';

// A filepath whose first segment is literally `partials` collides with the
// store's own `partials/` key prefix, which is what makes it a direct test of
// the response's prefix stripping rather than an incidental one.
const PREFIXED_TEMPLATE_FILEPATH = 'partials/prefix-check.html';
...
describe('PUT /publishing-api/v1/templates/partials/*filepath with happy path', ({ before, it }) => {
    ...
    before(async () => {
        ...
        url = new URL(`${ getBaseUrl() }/publishing-api/v1/templates/partials/${ TEMPLATE_FILEPATH }`);
    ...
    // The route segment is `partials`, the reported kind is `partial`, and the
    // store prefix is `partials/`. The reported kind is the handler's own closure
    // variable, fixed when createPutTemplateHandler('partial') is called, so this
    // assertion pins the route-to-handler wiring in routes/publishing-api-v1.js —
    // where the pattern and the handler are named independently — rather than
    // which store prefix the write eventually landed on.
    it('returns the kind, filepath, and buildId in the JSON:API attributes', () => {
        assertEqual('partial', body.data.attributes.kind);
...
describe('PUT /publishing-api/v1/templates/partials/*filepath twice for the same filepath', ({ before, it }) => {
    ...
    before(async () => {
        // Construct the URL here so the test fails if it is invalid
        // instead of crashing the whole test run.
        url = new URL(`${ getBaseUrl() }/publishing-api/v1/templates/partials/${ FLAT_TEMPLATE_FILEPATH }`);
    ...
    // This block also carries the single-segment filepath: the wildcard has to
    // match one segment as readily as the three the happy path sends, and a
    // flat name is what every application partial uses today.
    it('returns an identical JSON:API payload both times', () => {
        ...
        assertEqual(FLAT_TEMPLATE_FILEPATH, secondBody.data.attributes.filepath);
        assertEqual('partial', secondBody.data.attributes.kind);
        assertEqual(TEST_BUILD_ID, secondBody.data.attributes.buildId);
    });
});

describe('PUT /publishing-api/v1/templates/partials/*filepath with a partials-prefixed filepath', ({ before, it }) => {
    ...
    before(async () => {
        // Construct the URL here so the test fails if it is invalid
        // instead of crashing the whole test run.
        url = new URL(`${ getBaseUrl() }/publishing-api/v1/templates/partials/${ PREFIXED_TEMPLATE_FILEPATH }`);
    ...
    // (`partials/partials/prefix-check.html` here), but the URL path already
    // encodes the kind, so the response must report the prefix-less filepath
    // taken from the wildcard. Reporting the store key instead would double the
    // prefix.
    //
    // The doubling is not merely cosmetic for this kind: loadPartials() derives
    // each include name by stripping a single leading `partials/` from the
    // logical key, so this partial is included as
    // `{{>partials/prefix-check.html}}`. A client that trusted a doubled filepath
    // would name it wrong twice over.
...
describe('PUT /publishing-api/v1/templates/partials/*filepath with a mixed case filepath', ({ before, it }) => {
    ...
    before(async () => {
        ...
        // instead of crashing the whole test run.
        mixedCaseUrl = new URL(`${ getBaseUrl() }/publishing-api/v1/templates/partials/${ MIXED_CASE_TEMPLATE_FILEPATH }`);
        const foldedUrl = new URL(`${ getBaseUrl() }/publishing-api/v1/templates/partials/${ FOLDED_TEMPLATE_FILEPATH }`);
    ...
    it('returns the filepath folded to lower case', () => {
        ...
        assertEqual('partial', mixedCaseBody.data.attributes.kind);
