...
import formatDate from '../../../lib/view-service/helpers/format-date.js';

describe('formatDate(): with empty, null, or undefined date', ({ it }) => {
    ...
    it('returns empty string for null', () => {
        const result = formatDate(null, {}, null);
        assertEqual('', result);
    });

    it('returns empty string for undefined', () => {
        const result = formatDate(null, {}, undefined);
        assertEqual('', result);
    });
});

describe('formatDate(): with string date input', ({ it }) => {
    it('formats ISO string with default format', () => {
        const result = formatDate(null, {}, '2017-06-14T07:00:00.000Z');
        assert(result.includes('6/14/2017'));
    });


    it('formats ISO string with zone option', () => {
        const result = formatDate(null, { format: 'ISO', zone: 'America/New_York' }, '2017-06-14T07:00:00.000Z');
        assert(result.includes('2017-06-14'));
    });

    it('formats ISO string with locale option', () => {
        const result = formatDate(null, { format: 'DATE_MONTH_DATE', locale: 'fr' }, '2017-06-14T07:00:00.000Z');
        assert(result.includes('14'));
    });

    it('returns invalid reason for invalid ISO string', () => {
        const result = formatDate(null, {}, 'invalid-date-string');
        assertEqual('Invalid date (unparsable) String(invalid-date-string)', result);
    });
});

describe('formatDate(): with number date input', ({ it }) => {
    it('formats millisecond timestamp with default format', () => {
        const timestamp = new Date('2017-06-14T07:00:00.000Z').getTime();
        const result = formatDate(null, {}, timestamp);
        assert(result.includes('6/14/2017'));
    });
    ...
    it('formats millisecond timestamp with zone option', () => {
        const timestamp = new Date('2017-06-14T07:00:00.000Z').getTime();
        const result = formatDate(null, { format: 'ISO', zone: 'America/New_York' }, timestamp);
        assert(result.includes('2017-06-14'));
    });

    it('formats millisecond timestamp with locale option', () => {
        const timestamp = new Date('2017-06-14T07:00:00.000Z').getTime();
        const result = formatDate(null, { format: 'DATE_MONTH_DATE', locale: 'fr' }, timestamp);
        assert(result.includes('14'));
    });

    it('returns invalid reason for NaN', () => {
        const result = formatDate(null, {}, NaN);
        assert(result.includes('Invalid'));
    });
});

describe('formatDate(): with Date object input', ({ it }) => {
    ...
    it('formats Date object with zone option', () => {
        const date = new Date('2017-06-14T07:00:00.000Z');
        const result = formatDate(null, { format: 'ISO', zone: 'America/New_York' }, date);
        assert(result.includes('2017-06-14'));
    });

    it('formats Date object with locale option', () => {
        const date = new Date('2017-06-14T07:00:00.000Z');
        const result = formatDate(null, { format: 'DATE_MONTH_DATE', locale: 'fr' }, date);
        assert(result.includes('14'));
    });
...
describe('formatDate(): with object date input', ({ it }) => {
    it('formats date object with default format', () => {
        const dateObj = { year: 2017, month: 6, day: 14, hour: 7, minute: 0, second: 0 };
        const result = formatDate(null, {}, dateObj);
        assert(result.includes('6/14/2017'));
    });
    ...
    it('formats date object with zone option', () => {
        const dateObj = { year: 2017, month: 6, day: 14, hour: 7, minute: 0, second: 0 };
        const result = formatDate(null, { format: 'ISO', zone: 'America/New_York' }, dateObj);
        assert(result.includes('2017-06-14'));
    });

    it('formats date object with locale option', () => {
        const dateObj = { year: 2017, month: 6, day: 14, hour: 7, minute: 0, second: 0 };
        const result = formatDate(null, { format: 'DATE_MONTH_DATE', locale: 'fr' }, dateObj);
        assert(result.includes('14'));
    });
...
describe('formatDate(): with invalid input types', ({ it }) => {
    it('returns invalid message for boolean true', () => {
        const result = formatDate(null, {}, true);
    ...
    it('returns invalid message for array', () => {
        const result = formatDate(null, {}, [ 1, 2, 3 ]);
        assert(result.includes('Invalid date'));
    });
});

describe('formatDate(): with custom formats', ({ it }) => {
    const testDate = new Date('2017-06-14T07:00:00.000Z');

    it('formats with ISO format', () => {
        const result = formatDate(null, { format: 'ISO', zone: 'utc' }, testDate);
        assertEqual('2017-06-14T07:00:00.000Z', result);
    });

    it('formats with ISO_DATE format', () => {
        const result = formatDate(null, { format: 'ISO_DATE', zone: 'utc' }, testDate);
        assertEqual('2017-06-14', result);
    });

    it('formats with UTC format', () => {
        const result = formatDate(null, { format: 'UTC', zone: 'utc' }, testDate);
        assertEqual('Wed, 14 Jun 2017 07:00:00 GMT', result);
    });

    it('formats with DATE_MONTH_DATE format', () => {
        const result = formatDate(null, { format: 'DATE_MONTH_DATE' }, testDate);
        assertEqual('Jun 14', result);
    });
});

describe('formatDate(): with Luxon preset formats', ({ it }) => {
    const testDate = new Date('2017-06-14T07:00:00.000Z');

    it('formats with DATE_SHORT preset', () => {
        const result = formatDate(null, { format: 'DATE_SHORT' }, testDate);
        assertEqual('6/14/2017', result);
    });

    it('formats with DATE_MED preset', () => {
        const result = formatDate(null, { format: 'DATE_MED' }, testDate);
        assertEqual('Jun 14, 2017', result);
    });

    it('formats with DATE_FULL preset', () => {
        const result = formatDate(null, { format: 'DATE_FULL' }, testDate);
        assertEqual('June 14, 2017', result);
    });

    it('formats with TIME_SIMPLE preset', () => {
        const result = formatDate(null, { format: 'TIME_SIMPLE', zone: 'America/New_York' }, testDate);
        assertEqual('3:00 AM', result);
    });

    it('formats with TIME_24_SIMPLE preset', () => {
        const result = formatDate(null, { format: 'TIME_24_SIMPLE', zone: 'America/New_York' }, testDate);
        assertEqual('03:00', result);
    });

    it('formats with DATETIME_MED preset', () => {
        const result = formatDate(null, { format: 'DATETIME_MED', zone: 'America/New_York' }, testDate);
        assertEqual('Jun 14, 2017, 3:00 AM', result);
    });

    it('formats with DATETIME_FULL preset', () => {
        const result = formatDate(null, { format: 'DATETIME_FULL', zone: 'America/New_York' }, testDate);
        assertEqual('June 14, 2017 at 3:00 AM EDT', result);
    });
});

describe('formatDate(): with combined zone and locale options', ({ it }) => {
    const testDate = new Date('2017-06-14T07:00:00.000Z');
