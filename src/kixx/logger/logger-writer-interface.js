 ...
 *   responsible for handing the entry to its output stream or destination. A
 *   platform may still buffer the physical write, so the interface does not
 *   guarantee that bytes are flushed to the OS before `write` returns
 ...
 *   Responsible for formatting, buffering, and routing the entry to its output
 *   stream or destination.
 *
 *   Parameters:
