...
- Superseded after completion (recorded for later agents): two design decisions above no longer describe the code. (1) The context-taking read delegates on `ContentAddressableStore` were removed; reads now go exclusively through `openSnapshot(context)`, leaving the facade a write/publishing surface. (2) `content-snapshot.js` is no longer the owner of the logical pathname layout; the four bundle constants and `normalizeTemplatePath()` / `normalizePagePath()` now live in `addressing.js` alongside `normalizePathname()`, which removes the write path's dependency on the read-path module. `content-snapshot.js` still owns the snapshot read API.
