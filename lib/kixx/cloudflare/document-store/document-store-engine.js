...
export default class DocumentStoreEngine {
     ...
     *
     * New definitions add a generated column backed by `json_extract()` and an
     * accompanying composite index on `(type, col)`. Definitions that have been
     * removed drop the index then the column. Safe to call on every startup — DDL
     * uses IF NOT EXISTS or is guarded by the PRAGMA pre-check.
     *
        ...
        for (const indexDefinition of this.#indexDefinitions) {
            ...
            if (!existingKeyColumns.includes(columnName)) {
                ...
                try {
                    ...
                    `).run();
                    // IF NOT EXISTS guards against the edge case where the column was created
                    // by a previous run that crashed before this index statement completed.
                    await db.prepare(
                        `CREATE ${ wantUnique ? 'UNIQUE ' : '' }INDEX IF NOT EXISTS ${ indexName } ON documents (type, ${ columnName })`,
                    ).run();
                } catch (cause) {
                    throw new Error(`Unable to add index column "${ columnName }" to D1 documents table`, { cause });
                }
            } else if (existingIndexUnique.has(indexName) && existingIndexUnique.get(indexName) !== wantUnique) {
     ...
     * @throws {AssertionError} When `options.index` is missing or not present in `indexDefinitions`, or when `equalTo` is combined with range bounds
    ...
    async query(context, type, options) {
        ...
        if (options.cursor && !isString(options.cursor)) {
            throw new AssertionError('DocumentStoreEngine#query() cursor must be an encoded string');
        }

        const limit = isNumberNotNaN(options.limit) ? options.limit : 100;

        const { sql, params } = this.#composeQueryStatement(options, type, columnName, limit);
        ...
        if (results.length > limit) {
            ...
            nextCursor = this.#encodeCursor(last.key, last.id);
        }
     ...
     * @returns {Promise<{records: Object[], cursor: string|null}>} Page of records and an opaque next-page cursor, or null on the last page
     * @throws {AssertionError} When `equalTo` is combined with range bounds
     * @throws {Error} When the D1 query fails
     */
    async scan(context, type, options) {
        ...
        if (options.cursor && !isString(options.cursor)) {
            throw new AssertionError('DocumentStoreEngine#scan() cursor must be an encoded string');
        }
     ...
     * @throws {AssertionError} When `version` is not a positive number
    ...
    async update(context, doc, version) {
        if (!isNumberNotNaN(version) || version <= 0) {
            throw new AssertionError('DocumentStoreEngine#update() requires a positive version number');
        }
     ...
     * - **Force delete** (`version` is absent, zero, or NaN): Deletes unconditionally.
     *   Returns `false` (rather than throwing) when no matching row exists.
     *
     * @param {Object} context - Cloudflare Workers execution context
     * @param {string} type - Document type
     * @param {string} id - Document identifier
     ...
     * message includes the offending column reference, e.g.
     * `UNIQUE constraint failed: documents.key_virtual_file_workspace_path`.
     * The regex is anchored to the `documents.key_*` shape so it cannot match
     * unrelated errors.
     *
    ...
    #translateUniqueConflict(type, err) {
        ...
        const match = /UNIQUE constraint failed:\s*documents\.(key_[A-Za-z0-9_]+)/.exec(message);
        if (!match) {
            return null;
        }
        const indexName = this.columnNameToKey(match[1]);
    ...
    #composeQueryStatement(options, type, columnName, limit) {
        ...
        if (options.cursor) {
            const { v: cursorValue, id: cursorId } = this.#decodeCursor(options.cursor);
