...
export default function PageHandler(options) {
    ...
    return async function pageHandler(context, request, response) {
        ...
        const [ body, template ] = await Promise.all([
            ...
            viewService.getBaseTemplate(props.baseTemplateId),
        ]);
        ...
        if (!contentType && isNonEmptyString(props.baseTemplateId)) {
            const extname = path.extname(props.baseTemplateId);
            if (extname) {
        ...
        return response.respondWithUTF8(200, html, { contentType });
    };
}
