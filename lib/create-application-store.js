				...
				for (let n = items.length - 1; n >= 0; n--) {
					...
					if (!isNonEmptyString(type)) {
						return new ProgrammerError(
							`Missing relationship key type in relationship '${relationship}'`
						);
					}
					const Model = models[type];
					if (!Model) {
						return new ProgrammerError(
							`No relationship model for type '${type}'`
						);
					}
					...
					if (Boolean(Model.global) !== isGlobal) {
						if (Model.global) {
							return new ProgrammerError(
								`A non global resource cannot be related to a global resource`
							);
						}
						return new ProgrammerError(
							`A global resource cannot be related to a non global resource`
						);
					}
				}
			}
		...
		return Object.defineProperties(Object.create(null), {
			create: {
				...
				value: function create(transaction, args) {
					...
					const relError = checkRelationships(Model.global, relationships);
					if (relError) {
						throw new ProgrammerError(
							`${relError.message} for type '${type}' in '${storeName}' create()`
						);
					}
			...
			update: {
				...
				value: function update(transaction, args) {
					...
					if (relError) {
						throw new ProgrammerError(
							`${relError.message} for type '${type}' in '${storeName}' update()`
						);
					}
