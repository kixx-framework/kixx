...
module.exports = defineFunction({
	...
	func: function createApplicationStore(globalConfig, storeName, modelConfigs, api) {
		...
		MIDDLEWARE.forEach((name) => {
			if (!Array.isArray(globalConfig[name])) {
				throw new ProgrammerError(
					`Invalid or missing global middleware Array for "${name}" in globalConfig.`
				);
			}
		});
		...
		const models = modelConfigs.reduce((models, config, i) => {
			...
			const middleware = MIDDLEWARE.reduce((middleware, name) => {
				...
				const globalSeed = globalConfig[name];
				if (globalSeed && !Array.isArray(globalSeed)) {
					throw new ProgrammerError(
						`Invalid middleware configuration (non-array) for "${name}" in global configuration`
					);
				}
