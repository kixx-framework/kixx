...
module.exports = defineFunction({
	...
	func: function createApplicationStore(globalConfig, storeName, modelConfigs, api) {
		...
		if (!isNonEmptyString(storeName)) {
			throw new ProgrammerError(`createApplicationStore() requires a valid storeName String`);
		}

		const models = modelConfigs.reduce((models, config) => {
			const {type} = config;

			const middleware = MIDDLEWARE.reduce((middleware, name) => {
				if (config[name] && !isFunction(config[name])) {
					throw new ProgrammerError(`Invalid middleware (non-Function value) configured for "${name}"`);
				}
