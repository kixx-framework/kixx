		...
		const models = modelConfigs.reduce((models, config, i) => {
			...
			const middleware = MIDDLEWARE.reduce((middleware, name) => {
				...
				if (modelSeed) {
					...
					if (isNotFunction(fn)) {
						throw new ProgrammerError(
							`Invalid store middleware function for '${name}' in type '${type}'`
						);
					}
				} else {
				...
				middleware[name] = createMiddleware(api, fn, options);
				return middleware;
			}, Object.create(null));
...
function createMiddleware(api, fn, options) {
	options = options || Object.create(null);

	return function (args, resolve, reject) {
		return fn(api, options, args, resolve, reject);
	};
}
