...
const {StackedError, ProgrammerError, StoreArgs} = require(`../index`);
const {isFunction, isNumber, isNonEmptyString, omit} = require(`../library`);
		...
		return Object.defineProperties(Object.create(null), {
			create: {
				...
				value: function create(transaction, args) {
					const {scope, type, id, attributes, relationships, meta} = args.object;
					const Model = models[type];

					if (!isNonEmptyString(scope)) {
						throw new ProgrammerError(
							`Invalid args.scope passed to "${storeName}" create()`
						);
					}
					if (!isNonEmptyString(type)) {
						throw new ProgrammerError(
							`Invalid args.type passed to "${storeName}" create()`
						);
					}
					if (!Model) {
						throw new ProgrammerError(
			...
			update: {
				...
				value: function update(transaction, args) {
					const {object} = args;
					const {scope, type, id, attributes, relationships, meta} = object;
					const Model = models[type];
			...
			scan: {
				...
				value: function scan(transaction, args) {
					...
					const limit = args.limit && isNumber(args.limit) ? args.limit : 10;
					const Model = models[type];

					if (!isNonEmptyString(scope)) {
						throw new ProgrammerError(
							`Invalid args.scope passed to "${storeName}" scan()`
						);
					}
					if (!isNonEmptyString(type)) {
						throw new ProgrammerError(
