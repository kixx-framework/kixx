...
export default class StaticFileServerStore {
     ...
     * @param {HttpServerRequest} request - Request object with a getReadStream() method that returns a readable stream
     * @param {Function} request.getReadStream - Method that returns a Node.js readable stream for the incoming file data
     ...
     */
    async putFile(pathname, request) {
        const filepath = this.#pathnameToFilepath(pathname);
        ...
        const incomingStream = request.getReadStream();
