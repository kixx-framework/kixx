...
export function HyperviewHandler(options) {
    ...
    async function hyperviewHandler(context, request, response) {
        ...
        const pathParts = validateAndSplitPathname(pathname);

        const page = await service.getPageData(request, response, pathParts, { useCache });
        ...
        const [ body, template ] = await Promise.all([
            service.getPageMarkup(pathParts, page, { useCache }),
    ...
    async function getStaticFile(context, request, response) {
        ...
        let useFileCache = false;
        if (isBoolean(options.useFileCache)) {
            useFileCache = options.useFileCache;
        } else if (isBoolean(config.useFileCache)) {
        ...
        // May throw a BadRequestError
        const parts = validateAndSplitPathname(pathname);
        ...
        const file = await service.getStaticFile(filepath, { useFileCache });
        ...
        if (ifNoneMatch) {
            const etag = await file.computeEtag();
        ...
        const contentType = getContentTypeFromFilepath(file.name) || 'application/octet-stream';
    ...
    function validateAndSplitPathname(pathname) {
