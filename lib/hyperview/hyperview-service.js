...
export default class HyperviewService {
    ...
    async getPageData(request, response, pathParts, options) {
        ...
        if (!pageExists) {
            pageExists = await this.#pageStore.doesPageExist(pathParts);
        }
        if (!pageExists) {
            return null;
        }
        ...
        if (!pageData) {
            pageData = await this.#pageStore.getPageData(pathParts);
    ...
    async getPageMarkup(pathParts, pageData, options) {
        const { useCache } = options || {};
        ...
        if (!template) {
            const file = await this.#pageStore.getPageTemplate(pathParts);
            const source = await file.read({ encoding: 'utf8' });

            const templateId = `${ pathParts.join('/') }/${ file.name }`;
            const includePartials = true;
            template = this.#templateEngine.compileTemplate(templateId, source, { includePartials });
        ...
        const content = await this.getPageMarkdown(pathParts, pageData, { useCache });
    ...
    async getPageMarkdown(pathParts, pageData, options) {
        const { useCache } = options || {};
        ...
        if (!markdownTemplates) {
            ...
            const markdownFiles = await this.#pageStore.getMarkdownContent(pathParts);
            ...
            for (const file of markdownFiles) {
                ...
                const templateId = `${ pathParts.join('/') }/${ file.name }`;
                const includePartials = true;
                const template = this.#templateEngine.compileTemplate(templateId, source, { includePartials });
    ...
    async getBaseTemplate(templateId, options) {
        ...
        if (!template) {
            const file = await this.#pageStore.getBaseTemplate(templateId);
    ...
    async getStaticFile(pathParts) {
        const file = await this.#staticFileServer.get(pathParts);
        return file;
    }
    ...
    #hydrateMetadataTemplate(templateId, templateSource, props) {
        ...
        const template = this.#templateEngine.compileTemplate(templateId, templateSource, { includePartials });
