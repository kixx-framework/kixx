...
export default class HyperviewService {
     ...
     * Includes built-in helpers (format_date, plus_one) and custom helpers loaded from files
    ...
    async getPageMarkdown(pathname, pageData, options) {
        ...
        if (!markdownTemplates) {
            ...
            for (const { filename, source } of markdownFiles) {
                ...
                const template = await this.#getMarkdownTemplate(pathname, filename, source);
     ...
     *
     * Partials are loaded once and cached for the lifetime of the service instance since
     * they don't change during runtime. Partials can reference other partials, enabling
     * recursive partial inclusion (e.g., header includes nav).
     * @async
