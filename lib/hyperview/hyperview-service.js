...
export default class HyperviewService {
    ...
    async getPageMarkup(pathParts, pageData, options) {
        ...
        if (!template) {
            template = await this.#getPageTemplate(pathParts);
    ...
    async getPageMarkdown(pathParts, pageData, options) {
        ...
        if (!markdownTemplates) {
            ...
            for (const file of markdownFiles) {
                const template = this.#getMarkdownTemplate(pathParts, file);
    ...
    async #getPageTemplate(pathParts) {
    ...
    async #getMarkdownTemplate(pathParts, file) {
        const templateId = `${ pathParts.join('/') }/${ file.name }`;
        const source = await file.read({ encoding: 'utf8' });
        const helpers = this.#templateStore.helpers;
        const partials = await this.#templateStore.loadPartials();
