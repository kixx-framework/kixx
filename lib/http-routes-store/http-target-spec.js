...
export default class HttpTargetSpec {
    ...
    toHttpTarget(vhost, route) {
        ...
        // Format: routeName/targetName (e.g., "Dashboard/Contexts/ViewContext")
        const name = `${ route.name }/${ this.name }`;
    ...
    static validateAndCreate(spec, routeName) {
        ...
        assertNonEmptyString(spec.name, `target.name is required and must be a non-empty string (in route: ${ routeName })`);
        ...
        if (spec.tags) {
            assertArray(spec.tags, `target.tags must be an Array (in route: ${ routeName })`);

            for (const tag of spec.tags) {
                assertNonEmptyString(tag, `target.tags[] must be a non-empty string (in route: ${ routeName })`);
            }
        }
