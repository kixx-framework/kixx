...
export default class HttpRoutesStore {
    ...
    async #loadVhostSpecs() {
        ...
        const rootFiles = await this.#fileSystem.readDirectory(this.#app_directory);
        ...
        try {
            ...
            throw new WrappedError(
                `Error reading virtual hosts config file: ${ cause.message } (${ filepath })`,
                { cause },
                this.loadVirtualHosts
            );
        }
    ...
    /**
     * Loads route configurations from a single route file referenced by pathname.
     *
     * Resolves the pathname to a filesystem path relative to the routes directory, reads the
     * route configuration file, and validates each route specification. Returns an array of
     * validated route configuration objects with enhanced error messages including
     * route index context.
     *
     * @param {string} pathname - Pathname referencing the route configuration file (e.g., "api/v1/users.jsonc")
     ...
     */
    async #loadRoutesConfigsFile(pathname, sourceFilepath) {
        if (!isNonEmptyString(pathname)) {
            throw new ValidationError(
                `vhost routes must be an Array of pathname strings (${ sourceFilepath })`,
                null,
                this.loadVirtualHosts
            );
        }
        ...
        if (!filepath) {
            throw new ValidationError(
                `Invalid routes pathname "${ pathname }" in vhost config (${ sourceFilepath })`,
                null,
                this.loadVirtualHosts
            );
        }
        ...
        try {
            ...
            throw new WrappedError(
                `Error reading HTTP routes config file: ${ cause.message } (${ filepath })`,
                { cause },
                this.loadVirtualHosts
            );
        }
        ...
        return routeConfigs.map((config, index) => {
            try {
                return HttpRouteSpec.validateAndCreate(config);
            } catch (cause) {
                throw new ValidationError(
                    `${ cause.message }: ${ pathname }#routes[${ index }]`,
                    { cause },
                    this.loadVirtualHosts
                );
            }
    ...
    /**
     * Converts a route pathname to a filesystem filepath within the routes directory.
     *
     * Resolves the pathname relative to the routes directory and performs security validation
     * to prevent path traversal attacks. Returns null if the resolved path is outside the
     * routes directory.
     *
     * @param {string} pathname - Route pathname (e.g., "api/v1/users.jsonc")
     * @returns {string|null} Resolved filepath within routes directory, or null if path traversal detected
     */
    #pathnameToFilepath(pathname) {
        ...
        const filepath = path.join(this.#routes_directory, ...parts);

        // Security check: ensure the resolved filepath is within the base directory.
        // We compute the relative path from base dir to the filepath - if it starts with
        // '..' or is absolute, the directory is outside the base directory (path traversal).
        const resolvedFilepath = path.resolve(filepath);
        const relativePath = path.relative(this.#routes_directory, resolvedFilepath);
