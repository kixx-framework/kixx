/**
 * Node.js filesystem adapter — the concrete implementation of the Filesystem
 * port using Node.js built-in fs and fs/promises APIs.
 *
 * @see {import('../../ports/filesystem.js').Filesystem} Filesystem port
...
/**
 * @typedef {import('../ports/filesystem.js').DirEntry} DirEntry
 */

/**
 * @typedef {import('../ports/filesystem.js').FileStats} FileStats
 */
...
export function createReadStream(filepath, options = {}) {
    // fs.createReadStream() does not throw ENOENT synchronously — it returns
    // a stream that emits an error event when read. Check existence first so
    // the port contract (return null for missing files) can be satisfied.
