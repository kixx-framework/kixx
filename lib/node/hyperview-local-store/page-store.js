...
import { isNonEmptyString, assertNonEmptyString, assert } from '../../assertions.js';
 ...
 *
 * @see {import('../../ports/hyperview-page-store.js').HyperviewPageStore} HyperviewPageStore port
 */
export default class PageStore {
     ...
     * @param {string} options.directory - Absolute or relative path to the base directory containing page directories
    ...
    constructor(options) {
        ...
        assert(options.fileSystem, 'PageStore fileSystem must be provided');
    ...
    async getPageTemplate(pathname, options) {
        ...
        if (isNonEmptyString(templateFilename)) {
            // Use the explicit template filename if provided
            filename = templateFilename;
        } else {
            // Otherwise, search for default page.html or page.xml
            const files = await this.#fileSystem.readDirectory(dirpath);
        ...
        const source = await this.#fileSystem.readUtf8File(path.join(dirpath, filename));
    ...
    async getMarkdownContent(pathname) {
        ...
        const markdownFiles = files.filter((f) => {
            return f.isFile && MARKDOWN_FILE_PATTERN.test(f.name);
        });
