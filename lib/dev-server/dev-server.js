...
export default class DevServer {
    ...
    startServer(logger, config, port) {
        ...
        server.on('connection', function onServerConnection(socket) {
            ...
            logger.info('unencrypted server connection', { remoteAddress });
        });

        server.on('dropRequest', function onDropRequest() {
            ...
            logger.error('unencrypted server will drop request; socket limit reached');
        });

        server.once('listening', function onServerListening() {
            const addr = server.address();
            logger.info('dev server listening', { port: addr.port });
        });
