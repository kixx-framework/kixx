...
export default class DevServer {
        ...
        const proxyRequest = http.request(options, (proxyResponse) => {
            proxyResponse.once('error', (error) => {
                ...
                if (error.code === 'ECONNRESET') {
                    ...
                    // Send error response to client if headers haven't been sent yet
                    if (!res.headersSent) {
                        this.sendBadGatewayResponse(req, res, 'Backend response error');
                    }
                }
            });
            ...
            if (error.code === 'ECONNRESET') {
                ...
                if (!res.headersSent) {
                    if (error.code === 'ECONNREFUSED') {
                        this.sendServiceUnavailableResponse(req, res, 'Backend server is not responding');
                    } else if (error.code === 'ENOTFOUND') {
                        this.sendBadGatewayResponse(req, res, 'Backend host not found');
                    } else if (error.code === 'ETIMEDOUT') {
                        this.sendGatewayTimeoutResponse(req, res, 'Backend request timed out');
                    } else {
                        this.sendBadGatewayResponse(req, res, 'Backend connection failed');
                    }
                }
            }
        });

        // Handle client abort to clean up backend request
        req.on('aborted', () => {
            logger.info('client request aborted', { method, path, id });
            proxyRequest.destroy();
        });

        // Handle client close to clean up backend request
        req.on('close', () => {
            if (!res.headersSent) {
                logger.info('client connection closed before response', { method, path, id });
                proxyRequest.destroy();
            }
        });
    ...
    checkURL(hostname, pathname) {
        ...
        if (!pathname || typeof pathname !== 'string') {
            throw new TypeError('Invalid pathname: must be a non-empty string');
        }
    ...
    sendInvalidUrlResponse(req, res, message = 'Invalid request URL path') {
        this.sendErrorResponse(req, res, 400, message + '\n');
    }

    sendServiceUnavailableResponse(req, res, message = 'Service Unavailable') {
        this.sendErrorResponse(req, res, 503, message + '\n');
    }

    sendBadGatewayResponse(req, res, message = 'Bad Gateway') {
        this.sendErrorResponse(req, res, 502, message + '\n');
    }

    sendGatewayTimeoutResponse(req, res, message = 'Gateway Timeout') {
        this.sendErrorResponse(req, res, 504, message + '\n');
    }
