...
export function createHttpRequestHandler(logger, targetPort, host, protocol) {
    ...
    return function httpRequestHandler(req, res) {
        ...
        let url;
        try {
            url = createFullURL(targetPort, path);
        } catch (error) {
        ...
        logger.info('proxy request', { method, path, id, host });
