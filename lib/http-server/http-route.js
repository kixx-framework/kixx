...
export default class HttpRoute {
    ...
    /**
     * The URL pathname pattern string used to create this route
     * @type {string}
     */
    #pattern = null;
    ...
    /**
     * Reverse constructs the pathname and preferred HTTP method for a target.
     *
     * Takes a target identifier in the format "RouteName/TargetName" and pathname
     * parameters, then compiles them into a concrete pathname and selects the
     * preferred HTTP method for that target.
     *
     * @param {string} targetId - Target identifier in format "RouteName/TargetName"
     ...
     * @throws {AssertionError} When route uses wildcard pattern (cannot be reversed)
     * @throws {AssertionError} When targetId format is invalid or route name doesn't match
     * @throws {AssertionError} When target is not found on this route
     */
    compileTargetPathname(targetId, params) {
        if (this.#pattern === '*') {
        ...
        // Parse targetId to extract route name and target name
        const separatorIndex = targetId.indexOf('/');

        if (separatorIndex === -1) {
            throw new AssertionError(
                `Invalid targetId format "${ targetId }"; expected "RouteName/TargetName"`
            );
        }

        const routeName = targetId.substring(0, separatorIndex);
        const targetName = targetId.substring(separatorIndex + 1);

        if (routeName !== this.name) {
            throw new AssertionError(
                `Route name mismatch: targetId "${ targetId }" does not match route "${ this.name }"`
            );
        }
        ...
        if (!target) {
            throw new AssertionError(
                `Target "${ targetName }" not found on route "${ this.name }"`
            );
        }
        ...
        let method = target.allowedMethods[0];

        for (const preferredMethod of methodPriority) {
            if (target.allowedMethods.includes(preferredMethod)) {
