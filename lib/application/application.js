    ...
    async loadUserClassesFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            const rolesFile = files.find((entry) => {
                return entry.isFile() && /.roles.jsonc?$/.test(entry.name);
            });
    ...
    async loadCollectionsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            if (classFile) {
                ...
                assertNonEmptyString(CollectionConstructor.TYPE, `Expected Collection TYPE class member in ${ classFilepath }`);
                const key = `${ plugin.name }.${ CollectionConstructor.TYPE }`;
