 ...
 * initialize() method to set up the application, which returns an ApplicationContext
...
export default class Application {
    ...
    /**
     * Application context instance providing access to registered services and collections.
     * @type {ApplicationContext|null}
     */
    #context = null;
     ...
     * @public
     * @returns {ApplicationContext|null} The application context instance, or null if not yet initialized
     */
    get context() {
        return this.#context;
    }
    ...
    /**
     * Bootstraps the application by loading configuration and creating core services.
     * Does not initialize plugins — callers should invoke initializePlugins() separately.
     * @async
     * @public
     ...
     * @returns {Promise<ApplicationContext>} The initialized application context
    ...
    async loadConfiguration(options = {}) {
        ...
        // NOTE: Loading the configs may reset the #applicationDirectory based
        //       on the config file location.
        const configs = await this.#loadLatestConfig();
        const secrets = await this.#loadLatestSecrets();
     ...
     * in alphabetical order by directory name, with the app plugin loaded last to allow overrides.
     * Each plugin goes through registration (services) then initialization (async setup) phases.
     * @async
     * @public
     * @param {ApplicationContext} [context] - Application context for plugin registration. Defaults to this.#context
    ...
    async initializePlugins(context) {
        ...
        const directories = entries
            ...
            .sort(); // Sort alphabetically for deterministic load order across platforms
     ...
     * @param {ApplicationContext} context - Application context for registration
     * @param {Plugin} plugin - Plugin containing collection definitions
     */
    #registerCollectionsFromPlugin(context, plugin) {
        context = context || this.#context;
        assert(context, 'An ApplicationContext object must be provided to registerCollectionsFromPlugin()');
    ...
    /**
     * Loads configuration from the #configFilepath if it exists. Discovers and sets
     * the #configFilepath if it does not exist. Sets #applicationDirectory as a
     * side effect, if derived from the config file location.
     * @async
    ...
    async #loadLatestConfig() {
        ...
        if (isNonEmptyString(this.#configFilepath)) {
            values = await this.#fileSystem.readJSONFile(this.#configFilepath);
            if (!values) {
            ...
            if (!isNonEmptyString(this.#applicationDirectory)) {
                this.#applicationDirectory = path.dirname(this.#configFilepath);
            }
            return values;
        }
        ...
        if (file) {
            this.#configFilepath = path.join(this.#applicationDirectory, file.name);
            values = await this.#fileSystem.readJSONFile(this.#configFilepath);
        } else {
    ...
    /**
     * Loads secrets from the #secretsFilepath if it exists. Discovers and sets the
     * #secretsFilepath if it does not yet exist. Returns empty object if no
     * secrets file exists (secrets are optional).
     * @async
     * @returns {Promise<Object>} Parsed secrets object, or empty object if not found
     */
    async #loadLatestSecrets() {
        let values = {};

        if (isNonEmptyString(this.#secretsFilepath)) {
            const result = await this.#fileSystem.readJSONFile(this.#secretsFilepath);
            ...
            if (!isNonEmptyString(this.#applicationDirectory)) {
                this.#applicationDirectory = path.dirname(this.#secretsFilepath);
            }
        ...
        if (file) {
            this.#secretsFilepath = path.join(this.#applicationDirectory, file.name);
            values = await this.#fileSystem.readJSONFile(this.#secretsFilepath);
        }
