...
export default class Application {
    ...
    async initialize(options) {
        ...
        // We need the application directory to be set by the
        // config loader before creating the paths object.
    ...
    registerViewsFromPlugin(context, plugin) {
        for (const { ViewConstructor, schema } of plugin.views) {
            const view = new ViewConstructor(context, schema);
            const key = `${ plugin.name }.${ ViewConstructor.name }`;
            context.registerCollection(key, view);
        }
    }

    registerMiddlewareFromPlugin(plugin) {
        for (const fn of plugin.middleware) {
            // Put the middleware name in the plugin namespace.
            const key = `${ plugin.name }.${ fn.name }`;
            registerMiddleware(key, fn);
        }

        for (const fn of plugin.requestHandlers) {
            // Put the middleware name in the plugin namespace.
            const key = `${ plugin.name }.${ fn.name }`;
            registerHandler(key, fn);
        }

        for (const fn of plugin.errorHandlers) {
            // Put the middleware name in the plugin namespace.
            const key = `${ plugin.name }.${ fn.name }`;
    ...
    async loadLatestConfig(filepath) {
        ...
        const file = entries.find((entry) => {
            return entry.isFile() && /^kixx-config.jsonc?$/.test(entry.name);
        });

        if (file) {
            const filepath = path.join(this.#applicationDirectory, file.name);
            try {
                values = await this.#fileSystem.readJSONFile(filepath);
            } catch (cause) {
                throw new WrappedError(`Error in config JSON file at ${ filepath }`, { cause });
            }
        } else {
            throw new WrappedError(
    ...
    async loadLatestSecrets(filepath) {
        ...
        // Accept both .json and .jsonc file extensions.
        const file = entries.find((entry) => {
            return entry.isFile() && /^.secrets.jsonc?$/.test(entry.name);
        });
        ...
        if (file) {
            ...
            try {
                ...
                throw new WrappedError(`Error in secrets JSON file at ${ filepath }`, { cause });
            }
        }
