...
import RootUser from '../user/root-user.js';
...
export default class Application {
    ...
    async initialize(options) {
        ...
        const rootUser = new RootUser();
        const context = this.createContext(runtime, config, paths, logger, rootUser);
     ...
     * @param {RootUser} rootUser - The RootUser instance
     * @returns {Context} Application context
     */
    createContext(runtime, config, paths, logger, rootUser) {
    ...
    async loadCollectionsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            const classFile = files.find((entry) => {
                return entry.isFile() && /.schema.(js|mjs)$/.test(entry.name);
            });

            assert(schemaFile, `Expected a collection model schema file in ${ dir }`);
            assert(classFile, `Expected a collection class file in ${ dir }`);
            ...
            assert(CollectionConstructor.TYPE, `Expected Collection TYPE class member in ${ classFilepath }`);
    ...
    async loadFormsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            assert(classFile, `Expected a form class file in ${ dir }`);
            ...
            const form = new FormConstructor(context);
            assert(FormConstructor.NAME, `Expected Form NAME class member in ${ classFilepath }`);
