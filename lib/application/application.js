...
export default class Application {
    ...
    async initializePlugins(context) {
        ...
        const plugins = await pluginStore.loadPlugins(paths.app_plugin_directory);

        // Load plugin module definitions first, so they are available during
        // the plugin registration and initialization phases.
        for (const plugin of plugins) {
            // eslint-disable-next-line no-await-in-loop
            await this.loadUserClassesFromPlugin(context, plugin);
            // eslint-disable-next-line no-await-in-loop
            await this.loadCollectionsFromPlugin(context, plugin);
            // eslint-disable-next-line no-await-in-loop
            await this.loadFormsFromPlugin(context, plugin);
            // eslint-disable-next-line no-await-in-loop
            await this.loadViewsFromPlugin(context, plugin);
            // eslint-disable-next-line no-await-in-loop
    ...
    async loadUserClassesFromPlugin(context, plugin) {
        context = context || this.#context;
        assert(context, 'A Context object must be provided to loadUserClassesFromPlugin()');

        const { usersDirectory } = plugin;
        const entries = await this.#fileSystem.readDirectory(usersDirectory);

        const directories = entries
            .filter((entry) => entry.isDirectory())
            .map((entry) => path.join(usersDirectory, entry.name));
        ...
        for (const dir of directories) {
            ...
            const rolesFile = files.find((entry) => {
                return entry.isFile() && /.roles.json$/.test(entry.name);
            });

            const classFile = files.find((entry) => {
                return entry.isFile() && /.user.(js|mjs)$/.test(entry.name);
            });

            let roles = null;

            if (rolesFile) {
                const rolesFilepath = path.join(dir, rolesFile.name);
                try {
                    // eslint-disable-next-line no-await-in-loop
                    roles = await this.#fileSystem.readJSONFile(rolesFilepath);
                } catch (cause) {
                    throw new WrappedError(
                        `Unable to load Users roles from ${ rolesFilepath }`,
                        { cause }
                    );
                }
            }
            ...
            if (classFile) {
                ...
                try {
                    ...
                    throw new WrappedError(
                        `Unable to load User class from ${ classFilepath }`,
                        { cause }
                    );
                }

                const UserConstructor = mod.default;
                assertNonEmptyString(UserConstructor.TYPE, `Expected User TYPE class member in ${ classFilepath }`);
                const key = `${ plugin.name }.${ UserConstructor.TYPE }`;

                if (roles) {
                    UserConstructor.ROLES = roles;
                }

                context.registerUserType(key, UserConstructor);
            }
        }
    }
    ...
    async loadCollectionsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            const schemaFile = files.find((entry) => {
                return entry.isFile() && /.schema.json$/.test(entry.name);
            });
            ...
            if (classFile) {
                ...
                assertFunction(CollectionConstructor.TYPE, `Expected Collection TYPE class member in ${ classFilepath }`);
                assertNonEmptyString(CollectionConstructor.TYPE.name, `Expected Collection TYPE class to have a name in ${ classFilepath }`);
                const key = `${ plugin.name }.${ CollectionConstructor.TYPE.name }`;
    ...
    async loadFormsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            if (classFile) {
                ...
                assertNonEmptyString(FormConstructor.NAME, `Expected Form NAME class member in ${ classFilepath }`);
    ...
    async loadViewsFromPlugin(context, plugin) {
        ...
        for (const dir of directories) {
            ...
            if (classFile) {
                ...
                assertNonEmptyString(ViewConstructor.NAME, `Expected View NAME class member in ${ classFilepath }`);
    ...
    createRootUser(context, userType = 'app.Root') {
        context = context || this.#context;
        assert(context, 'A Context object must be provided to createRootUser()');

        let RootUser;
        try {
            RootUser = context.getUserType(userType);
        } catch (cause) {
            throw new WrappedError(
                `Unable to get "${ userType }" user constructor as root user. Ensure plugins have been loaded and the user type class "${ userType }" is defined.`,
                { cause },
                this.createRootUser
            );
        }

        return new RootUser(context);
    }
}
