...
export default class Plugin {
    ...
    async load() {
        ...
        this.users = await this.loadUserRoles();
    ...
    async loadCollections() {
        ...
        for (const CollectionConstructor of collections) {
            if (CollectionConstructor) {
                const name = CollectionConstructor.Model?.name || CollectionConstructor.User.name;
    ...
    async loadCollection(directory) {
        ...
        if (classFile) {
            ...
            if (!isFunction(CollectionConstructor.Model) && !isFunction(CollectionConstructor.User)) {
                throw new AssertionError(`Expected Collection Model or User class member in ${ filepath }`);
            }
