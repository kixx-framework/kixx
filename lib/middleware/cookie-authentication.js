...
export class AuthenticationSession {
    ...
    constructor(options, config) {
        ...
        if (this.refreshWindowEndSeconds && this.expirationWindowSeconds && this.expirationWindowSeconds < this.refreshWindowEndSeconds) {
            throw new AssertionError(`The CookieAuthentication expirationWindowSeconds (${ this.expirationWindowSeconds }) must be greater than or equal to refreshWindowEndSeconds (${ this.refreshWindowEndSeconds })`);
        }
    }
...
export default function CookieAuthentication(options) {
    ...
    return async function cookieAuthentication(context, request, response) {
        ...
        if (sessionId) {
            ...
            newSession = await userCollection.createSessionFromUser(user);
        } else {
