...
/**
 * Removes a directory and all its contents recursively from the filesystem.
 *
 * This operation is idempotent - if the directory doesn't exist, it succeeds silently without
 * throwing an error, allowing callers to use it as an "ensure deleted" operation.
 * @public
 * @async
 * @param {string} directory - Absolute or relative path to the directory
 * @returns {Promise<void>} Resolves when the directory has been removed or if it doesn't exist
 * @throws {Error} When removing the directory fails for reasons other than non-existence (e.g., permission denied)
 */
export async function removeDirectory(directory) {
    // force: true makes this operation idempotent - if the directory doesn't exist, it succeeds
    // silently rather than throwing ENOENT. This allows callers to use removeDirectory as an
    // "ensure deleted" operation without checking existence first.
    // recursive: true removes the directory and all its contents (files and subdirectories).
 ...
 *
 * This operation is idempotent - if the directory already exists, it succeeds silently
 * without throwing an error, allowing callers to use it as an "ensure exists" operation
 * before writing files.
 *
 * @public
 * @async
 * @param {string} directory - Absolute or relative path to the directory
 * @returns {Promise<void>} Resolves when the directory exists (created or already present)
 * @throws {Error} When creating the directory fails (e.g., permission denied, path is a file)
 */
export async function ensureDirectory(directory) {
    // recursive: true creates parent directories as needed and doesn't throw
    // if the directory already exists, making this operation idempotent
