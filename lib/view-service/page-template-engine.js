...
export default class PageTemplateEngine {
    ...
    loadHelpers() {
        ...
        // TODO: We're going to do an initialization sequence here instead

        this.#helpersLoadedPromise = this.#fileSystem
            .readDirectory(this.#helpersDirectory)
            .catch((cause) => {
                return Promise.reject(
                    new WrappedError(`Unable to read helpers directory ${ filepath }`, { cause })
                );
            })
            .then((entries) => {
                const promises = entries.map((entry) => {
                    ...
                    return this.loadHelperFromFilepath(filepath).catch((cause) => {
                        return Promise.reject(
                            new WrappedError(`Unable to load template helper from ${ filepath }`, { cause })
                        );
                    });
                });
    ...
    async loadHelperFromFilepath(filepath) {
        if (entry.isFile()) {
            const { name, helper } = await this.#fileSystem.importAbsoluteFilepath(filepath);
