...
export default class ViewService {
    ...
    // TODO: We need to test getPageMarkdown
    ...
    async getPageMarkdown(urlPathname, pageData) {
        // Template ID follows pattern: /path/to/page → /path/to/page/page.md
        const templateId = `${ this.#normalizePath(urlPathname) }/page.md`;
    ...
    // TODO: renderMarkupForError() should go in the pageErrorHandler()

    /**
     * Renders markup for an error, using custom error page if available
     * Falls back to a default error page if no custom error page is found.
     *
    ...
    /**
     * Attempts to render a custom error page for a given error
     * Uses the HTTP status code as the pathname to find a custom error page.
     *
     * @param {Error} error - The error object (should have httpStatusCode if possible)
     * @returns {Promise<string|null>} The rendered error page HTML, or null if not found
     * @throws {WrappedError} When error page processing fails
     */
    async getPageBodyForError(error) {
        ...
        // Load both page content and base template concurrently
    ...
    /**
     * Renders a default error page as fallback
     * Provides a simple HTML error page when no custom error page is available.
     *
     * @param {Error} error - The error object (should have httpStatusCode if possible)
     * @returns {string} Simple HTML error page
     */
