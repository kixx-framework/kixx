'use strict';

const R = require(`ramda`);
const kixxAssert = require(`kixx-assert`);
const UserError = require(`../types/user-error`);
const Request = require(`../types/request`);
const Response = require(`../types/response`);

const {isFunction} = kixxAssert.helpers;

exports.createRequestHandler = (router) => {
	const routes = router.routes;
	const errorHandlers = router.errorHandlers;
	const notFoundHandlers = router.notFoundHandlers;

	const findMatch = exports.findMatch(routes);
	const handleError = exports.handleError(errorHandlers);
	const handleNotFound = exports.handleNotFound(notFoundHandlers);

	return function handleRequest(req, res) {
		req = Request.create(req);
		res = Response.create(res);

		const match = findMatch(req.url.pathname);

		if (match) {
			req = req.setParams(match.params);

			try {
				match.route.handler(req, res, function finalNext(err) {
					if (err) {
						handleError(err, req, res);
					}
				});
			} catch (err) {
				handleError(err, req, res);
			}
		} else {
			handleNotFound(req, res);
		}
	};
};

exports.findMatch = R.curry(function findMatch(routes, path) {
	let i;
	let matches;

	for (i = 0; i < routes.length; i++) {
		const {regexp, keys} = routes[i];
		matches = regexp.exec(path);

		if (matches) {
			// If we matched a route, collect the parameters if there are any and return.
			const params = keys.reduce((params, key, i) => {
				const name = key.name;
				params[name] = matches[i + 1];

				// If the parameter is a number, parse it as such.
				if (/^[\d]+$/.test(params[name])) {
					params[name] = parseInt(params[name], 10);
				}

				// If the parameter is a float, parse it as such.
				if (/^[\d]+.[\d]+$/.test(params[name])) {
					params[name] = parseFloat(params[name]);
				}

				return params;
			}, Object.create(null));

			return {route: routes[i], params};
		}
	}

	return false;
});

exports.composeMiddleware = function composeMiddleware() {
	const middleware = Array.prototype.slice.call(arguments).reverse();

	middleware.forEach((fn, i) => {
		if (!isFunction(fn)) {
			throw new UserError(`Middleware at index ${i} is not a Function.`);
		}
	});

	return function composedHandler(request, response, complete) {
		const composition = middleware.reduce((nextNext, fn) => {
			return function next(err, req, res) {
				if (err) {
					return complete(err);
				}

				req = req || request;
				res = res || response;

				fn(req, res, nextNext);
			};
		}, complete);

		return composition(null, request, response);
	};
};

exports.handleError = R.curry(function handleError(handlers, err, req, res) {
	if (handlers.length < 1) {
		throw err;
	}

	handlers.forEach((handler) => {
		handler(err, req, res);
	});
});

exports.handleNotFound = R.curry(function handleNotFound(handlers, req, res) {
	if (handlers.length < 1) {
		throw new Error(`No notFoundHandlers available`);
	}

	handlers.forEach((handler) => {
		handler(req, res);
	});
});
