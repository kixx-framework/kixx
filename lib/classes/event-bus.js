...
const {isFunction, isString, isObject, isNonEmptyString, compact, clone, deepFreeze} = require(`../../library`);
...
class EventBus {
	...
	observe(type, pattern, handler) {
		if (!isNonEmptyString(type)) {
			throw new ProgrammerError(`EventBus#observe() requires type String as the first parameter.`);
		}
		if (!isString(pattern)) {
			throw new ProgrammerError(`EventBus#observe() requires pattern String as the second parameter.`);
		}
		if (!isFunction(handler)) {
			throw new ProgrammerError(`EventBus#observe() requires handler Function as the third parameter.`);
		}
