...
export default class BaseLogger {
     ...
     * @param {Function} options.printWriter - Function called with each formatted log line string; typically `process.stdout.write.bind(process.stdout)` on Node.js
     ...
     * @throws {AssertionError} When printWriter is not a function
     */
    constructor(options) {
        const { name, level, printWriter } = options || {};

        assertNonEmptyString(name, 'A logger must be provided with a name');
        assertFunction(printWriter, 'A logger must be provided with a printWriter function');
    ...
    /**
     * The writer function injected at construction time.
     * @public
     * @returns {Function}
     */
    get printWriter() {
