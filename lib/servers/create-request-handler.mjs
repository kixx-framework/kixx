...
export default function createRequestHandler(params) {
	...
	return function handleRequest(req, res) {
		...
		try {
			...
			logger.debug('invalid request url', { url: req.url, err });
			sendInvalidUrlResponse(req, res);
			return;
		}

		const appConfig = Configurations.findHostApplication(config.applications);
		...
		if (usePreferredHost && usePreferredPort) {
			...
			newLocation = composeRedirectLocation(preferredProtocol, preferredHost, req.url, preferredPort);
		} else if (usePreferredHost) {
			newLocation = composeRedirectLocation(preferredProtocol, preferredHost, req.url, originatingPort);
		} else if (usePreferredPort) {
			newLocation = composeRedirectLocation(preferredProtocol, hostname, req.url, preferredPort);
		}
