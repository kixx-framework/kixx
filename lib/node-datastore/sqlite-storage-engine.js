...
export default class SQLiteStorageEngine {
     ...
     * @param {string} type
     * @param {string} id
     * @param {number} [version] - When provided, the stored version must match.
     * @returns {Promise<boolean>} True if deleted, false if the document did not exist.
     * @throws {DocumentNotFoundError} On versioned delete when (type, id) does not exist.
     * @throws {VersionConflictError} On versioned delete when the stored version does not match.
     */
    async delete(type, id, version) {
        if (version === undefined) {
            const unversioned = this.#db.prepare(
                'DELETE FROM documents WHERE type = ? AND id = ?'
            ).run(type, id);
            return unversioned.changes > 0;
        }
    ...
    async query(type, options) {
        ...
        if (cursor) {
            ...
            appendCursorCondition(conditions, params, col, cursorValue, cursorId, descending);
        }
 ...
 *   - If cursor value is non-null: standard `(col < val OR (col = val AND id < lastId))`.
 */
function appendCursorCondition(conditions, params, col, cursorValue, cursorId, descending) {
    if (cursorValue === null) {
        if (descending) {
