...
export default class SQLiteStorageEngine {
    ...
    async configureIndexes(indexes) {
        ...
        for (const { attribute } of indexes) {
            const colName = `"idx_${ attribute }"`;
            const idxName = `idx_type_${ attribute }`;

            if (!existingColumns.has(`idx_${ attribute }`)) {
                ...
                this.#db.exec(
                    `ALTER TABLE documents ADD COLUMN ${ colName }`
                    + ` TEXT GENERATED ALWAYS AS (json_extract(doc, '$.${ attribute }')) VIRTUAL`
                );
            }

            this.#db.exec(
                `CREATE INDEX IF NOT EXISTS ${ idxName } ON documents (type, ${ colName })`
            );
        }
        ...
        for (const col of existingColumns) {
            ...
            const attribute = col.slice(4); // strip the 'idx_' prefix
            if (!desiredAttributes.has(attribute)) {
                this.#db.exec(`DROP INDEX IF EXISTS idx_type_${ attribute }`);
            }
        }
    }
     ...
     * @param {Object} [options]
     * @param {number} [options.version] - Required for updates; absent for creates.
     * @returns {Promise<import('../ports/storage-engine.js').DocumentRecord>}
     * @throws {DocumentAlreadyExistsError} On create when (type, id) already exists.
    ...
    async put(doc, options) {
        ...
        if (version === undefined) {
            return this.#createDocument(doc, serialized, sortKey, now);
        }
    ...
    async query(type, options) {
        ...
        const col = index ? `"idx_${ index }"` : 'sort_key';
    ...
    #createDocument(doc, serialized, sortKey, now) {
        try {
            this.#db.prepare(
                'INSERT INTO documents (type, id, sort_key, doc, version, created_at, updated_at)'
                + ' VALUES (?, ?, ?, ?, 1, ?, ?)'
            ).run(doc.type, doc.id, sortKey, serialized, now, now);
        } catch (cause) {
            // SQLite signals a UNIQUE constraint violation with ERR_SQLITE_ERROR and a
            // message containing "UNIQUE constraint failed".
            if (isUniqueConstraintError(cause)) {
                throw new DocumentAlreadyExistsError(doc.type, doc.id);
            }
            throw new WrappedError(
                `SQLiteStorageEngine: failed to create ${ doc.type }:${ doc.id }`,
                { cause }
            );
        }
