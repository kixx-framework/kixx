...
import { IndexNotConfiguredError } from './errors.js';

const TYPE_PATTERN = /^[A-Za-z][A-Za-z0-9_-]*$/;
// Attribute names are more restrictive than type names — no hyphens, because the
// name is used to generate a SQL column name (idx_<attribute>).
 ...
 * all inputs, maintains in-memory index configuration, and delegates every persistence
 * operation to a pluggable StorageEngine. Swapping engines requires no changes to
 * application code.
 *
 * Usage:
 * ```javascript
...
export default class DataStore {
    ...
    /**
     * In-memory index registry. Populated by configureIndexes() and consulted by
     * query() to validate index references without an engine round-trip.
     * @type {Map<string, Set<string>>}
     */
    #configuredIndexes = new Map();
    ...
    async configureIndexes(indexes) {
        ...
        // Update the engine first; only swap the in-memory registry on success so that
        // a thrown engine error does not leave the registry out of sync with engine state.
        ...
        for (const { type, attribute } of indexes) {
            ...
            newMap.get(type).add(attribute);
        }
        this.#configuredIndexes = newMap;
    }
     ...
     * - **No version** — create. Fails if a document with the same (type, id) already exists.
     * - **Version provided** — update. Fails if the stored version does not match.
     *
     * @public
     ...
     * @throws {import('./errors.js').DocumentAlreadyExistsError} On create when (type, id) exists.
     ...
     * @param {number} options.version - Expected current version. Must match to proceed.
     ...
     * @throws {import('./errors.js').VersionConflictError} When the stored version does not match.
     */
    ...
    async query(type, options) {
        ...
        // Validate custom index exists in the in-memory registry.
        if (index !== undefined) {
            const typeIndexes = this.#configuredIndexes.get(type);
            if (!typeIndexes || !typeIndexes.has(index)) {
                throw new IndexNotConfiguredError(type, index, {}, this.query);
            }
        }
