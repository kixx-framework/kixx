...
export default class DatastoreEngine {
    ...
    queryKeys(options) {
        ...
        const range = descending
            ? getDescendingIndexRange(sortedList, startKey, endKey)
            : getAscendingIndexRange(sortedList, startKey, endKey);
        ...
        items = range.slice(inclusiveStartIndex, exclusiveEndIndex);

        if (exclusiveEndIndex === range.length) {
            exclusiveEndIndex = null;
        }

        // Include full documents if requested
        if (includeDocuments) {
            for (let i = 0; i < items.length; i += 1) {
                const { documentKey } = items[i].documentKey;
                const document = structuredClone(this.#documentsMap.get(documentKey));
                items[i] = Object.assign({}, items[i], { document });
            }
        }
