...
export default class Datastore {
     ...
     *
     * @async
     * @param {Object} doc - The document to store (must be a plain object with type and id)
    ...
    async queryKeys(options) {
        ...
        if (startKey) {
            startKey = this.createPrimaryKey(type, startKey);
        }
        if (endKey) {
            endKey = this.createPrimaryKey(type, endKey);
        }
        if (key) {
            key = this.createPrimaryKey(type, key);
        }
    ...
    /**
     * Converts a type and id strong to a namespaced datastore key
     *
