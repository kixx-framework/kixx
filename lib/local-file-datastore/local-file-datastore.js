 ...
 * @property {string} type - Document type identifier
...
export default class LocalFileDatastore {
    ...
    /**
     * Registers a view definition for secondary index queries.
     * @public
     * @param {string} key - View identifier used when calling queryView()
     * @param {ViewSource} view - View object with a map(document, emit) function
     * @throws {AssertionError} When view is missing or does not have a map() function
     */
    setView(key, view) {
        view = view || {};
     ...
     * @throws {AssertionError} When type or id are not non-empty strings
     * @throws {AssertionError} When checkConsistency is true and _rev doesn't match stored revision (error.code is ConflictError.CODE)
    ...
    async scanItems(type, options) {
        ...
        if (!isUndefined(options.startKey)) {
        ...
        // Build index in memory each time to avoid staleness (see queryView for rationale)
    ...
    async queryView(viewId, options = {}) {
        ...
        // Build index in memory each time to avoid staleness; trades CPU for simplicity
        let index;
        try {
    ...
    #mapViewDocuments(view, includeDocuments) {
        ...
        for (const document of this.#documentsMap.values()) {
            ...
            view.map(document, function emit(key, value) {
                ...
                if (includeDocuments) {
                    item.document = structuredClone(document);
                }
