...
import { assertNonEmptyString, assert, isFunction } from '../assertions.js';
...
export default class ApplicationBootstrap {
     ...
     */
    constructor({ environment, bootstrap }) {
        assertNonEmptyString(environment, 'ApplicationBootstrap requires an environment string');
        assert(bootstrap, 'ApplicationBootstrap requires a bootstrap object');
    ...
    async bootstrapApplication({ runtime, plugins }) {
        ...
        const utils = this.#bootstrap.createUtils();
        const applicationContext = new ApplicationContext({ runtime, config, logger, utils });
    ...
    async loadConfig(configStore) {
        const config = new Config(configStore, this.environment, this.#bootstrap.applicationDirectory);
    ...
    /**
     * Closes all plugins in insertion order by calling plugin.close() if defined.
     * @public
    ...
    async close() {
        ...
        if (errors.length > 0) {
            throw new AggregateError(errors, 'One or more plugins failed to close');
        }
    }
    ...
    createHttpRouter(applicationContext, vhostsConfigs) {
        ...
        const { middleware, requestHandlers, errorHandlers } = applicationContext.getHttpRouterRegistries();
    ...
    /**
     * Creates an HTTP server through the platform bootstrap.
     * @public
     * @param {ApplicationContext} applicationContext - Fully bootstrapped application context
     * @param {number} [port] - Port override passed through to the platform bootstrap
     * @returns {*} Platform-specific HTTP server instance
     */
    createHttpServer(applicationContext, port) {
        return this.#bootstrap.createHttpServer(applicationContext, port);
    }
}
