 ...
 * - put() with no version MUST reject with DocumentAlreadyExistsError if (type, id) exists.
 ...
 * - delete() MUST reject with VersionConflictError on version mismatch; return false if not found.
 * - query() cursor tokens are opaque — engines may encode them however they choose,
 *   but must produce stable pagination even when new records are inserted between pages.
 *   Seek-based cursors (rather than OFFSET) are strongly recommended for backend portability.
 *
 ...
 * receive only the canonical range operators, never `beginsWith` or the alias forms.
 *
 ...
 *   - No options.version → create. Rejects with DocumentAlreadyExistsError if (type, id) exists.
 *   - options.version provided → update. Rejects with DocumentNotFoundError if not found.
 ...
 *   Delete a document by (type, id) with an optimistic version check.
 *   Rejects with VersionConflictError if the stored version does not match.
 ...
 *   The DataStore normalizes all input (resolves aliases, applies defaults, expands
 *   beginsWith) before calling this method.
 *
